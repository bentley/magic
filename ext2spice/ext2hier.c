...
void
esOutputHierResistor(hc, dev, trans, term1, term2, has_model, l, w, dscale)
    HierContext *hc;
    ...
    Transform *trans;	/* Coordinate transform for output */
{
	...
	case DEV_RES:
		...
		esOutputHierResistor(hc, dev, trans, gate, source, has_model,
			l, w, 2);
		fprintf(esSpiceF, "\n%c", devchar);
		if (gate->dterm_attrs)
		...
		esOutputHierResistor(hc, dev, trans, gate, drain, has_model,
			l, w, 2);
	    }
	    else
	    {
		esOutputHierResistor(hc, dev, trans, source, drain, has_model,
			l, w, 1);
	    }
	    break;
...
int
devMergeHierVisit(hc, dev, trans)
    HierContext *hc;
    ...
    Transform *trans;		/* Coordinate transform not used */
{
    ...
    if (esDistrJunct)
	devDistJunctHierVisit(dev, trans);
...
int 
devDistJunctHierVisit(hc, dev, trans)
    HierContext *hc;
    Dev *dev;			/* Dev to examine */
    Transform *trans;		/* Coordinate transform not used */
{
